...
import static talkme.util.ParquetJsonUtil.generateJsonStructure;
...
public class TableController {
    ...
    @Path("/structure")
    @Consumes(MediaType.APPLICATION_OCTET_STREAM)
    public Response correctJson(File parquetFile){
        try {
            ParquetParser parser = new ParquetParser(parquetFile, 0);
            Map<String, Object> res = generateJsonStructure(parser.getSchema());
            parser.close();
